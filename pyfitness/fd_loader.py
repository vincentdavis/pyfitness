...
def slots2dict(classobj: object) -> dict:
    """
    Converts __slots__ of a class object to a dictionary, excluding unknown attributes.

    This function inspects the __slots__ attribute of a given class object, which lists
    the instance variables and their values, and returns a dictionary of these variables and
    their corresponding values, excluding any attribute which name starts with 'unknown_'.

    Args:
        classobj (object): The class object to be converted to a dictionary.

    Returns:
        dict: A dictionary where the keys are the instance variable names and the
        values are their corresponding values from the provided class object,
        excluding attributes starting with 'unknown_'.

    Example:
        class Test:
            __slots__ = ['var1', 'var2', 'unknown_var']
            def __init__(self):
                self.var1 = 1
                self.var2 = 2
                self.unknown_var = 3
        ...
        obj = Test()
        print(slots2dict(obj))  # Outputs: {'var1': 1, 'var2': 2}
    """
...
def fit2csv(fitfile: str | pd.DataFrame, outfile=None):
    ...
    for c in date_columns:
        try:
            df[c] = df[c].apply(lambda a: datetime.strftime(a, "%Y-%m-%d %H:%M:%S") if not pd.isnull(a) else "")
        except:
            raise
    if outfile:
...
def fitfileinfo(fit, show_unkown=False):
    ...
    for d in fit:
        match d:
            ...
            case fitdecode.records.FitDataMessage():
                ...
                if d.name.lower() not in ["record", "event", "session", "activity", "lap"] and (
                    show_unkown or "unknown_" not in d.name.lower()
                ):
    ...
    fileinfo += "###Record Fields:\n"
    for field in record_fields:
        fileinfo += f"- {field}\n"
    fileinfo += "###Event Types:\n"
